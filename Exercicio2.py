...
# as funções u_{k} (listas) para um determinado x_{r}, e um vetor d que contém os valores
# do vetor dr (passado como parâmetro) entre os intervalos ti e tf (convertidos para índi-
# ces do vetor, conforme a discretização adotada) - sendo que estes dois últimos retornos
# são usados apenas para os cálculos do erro e do resíduo
def sistema_normal(K, n_t, c_square, t_i, t_f, x_k, dr, x_r=0.7, delta_x=0.01, T=1):
    delta_t = T / n_t
    n_x = int(1/delta_x)
    ...
    for k in range(K):
        c.append(produto_escalar(u_k[k], d, delta_t, n))
...
def main():
    ...
    B,c,u,d = sistema_normal(K, n_t, c_square, t_i, t_f, x_k, dr, x_r, delta_x, T)
    a = fatoracao_Cholesky(B, c)
    print('Solucao obtida pela fatoracao de Cholesky:')
    print(a)
...
# --------------------------------------------------------------------------
# Valores para o último item do exercício 2 (solução exata desconhecida)
#    n_t = 2000
#    c_square = 20
#    delta_x = 0.005
#    x_r = 0.7
...
#        x_k.append(0.1 + 0.025*(k-1))
#
#    B, c, u, _ = sistema_normal(K, n_t, c_square, t_i, t_f, x_k, dr, x_r, delta_x, T)
#    a = fatoracao_Cholesky(B, c)
#    print('Solucao obtida pela fatoracao de Cholesky (ordem do algoritmo):')
#    print(a)
#    a.sort()
#    print('Solucao obtida pela fatoracao de Cholesky (ordem crescente):')
#    print(a)
#    print('Residuo da fatoracao de Cholesky:')
#    print_residuo(K, u, a, d, n_t, t_i, t_f, T)
# --------------------------------------------------------------------------
